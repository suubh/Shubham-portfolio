    ...
    getSymbol(task, data = false) {
        ...
        if (task.isPending() && !data) {
            return ((_a = this.options) === null || _a === void 0 ? void 0 : _a.lazy) || this.getSelfOrParentOption(task, 'showSubtasks') !== false && task.hasSubtasks() && !task.subtasks.every((subtask) => !subtask.hasTitle())
                ? chalk_1.default.yellow(this.figures.pointer)
                : chalk_1.default.yellowBright(this.spinner[this.spinnerPosition]);
        }
        else if (task.isCompleted() && !data) {
            return task.hasSubtasks() && task.subtasks.some((subtask) => subtask.hasFailed()) ? chalk_1.default.yellow(this.figures.warning) : chalk_1.default.green(this.figures.tick);
        }
        else if (task.isRetrying() && !data) {
            return ((_b = this.options) === null || _b === void 0 ? void 0 : _b.lazy) ? chalk_1.default.keyword('orange')(this.figures.warning) : chalk_1.default.keyword('orange')(this.spinner[this.spinnerPosition]);
        }
        else if (task.isRollingBack() && !data) {
            return ((_c = this.options) === null || _c === void 0 ? void 0 : _c.lazy) ? chalk_1.default.red(this.figures.warning) : chalk_1.default.red(this.spinner[this.spinnerPosition]);
        }
        else if (task.hasRolledBack() && !data) {
            return chalk_1.default.red(this.figures.arrowLeft);
        }
        else if (task.hasFailed() && !data) {
            return task.hasSubtasks() ? chalk_1.default.red(this.figures.pointer) : chalk_1.default.red(this.figures.cross);
        }
        else if (task.isSkipped() && !data && this.getSelfOrParentOption(task, 'collapseSkips') === false) {
            return chalk_1.default.yellow(this.figures.warning);
        }
        else if (task.isSkipped() && (data || this.getSelfOrParentOption(task, 'collapseSkips'))) {
            return chalk_1.default.yellow(this.figures.arrowDown);
        }
        return !data ? chalk_1.default.dim(this.figures.squareSmallFilled) : this.figures.pointerSmall;
    }
