...
const rootDbFile = process.env.NODE_ENV === `test` ? `test-datastore-${process.env.JEST_WORKER_ID}` : `datastore`;
let rootDb;
let databases;
...
function iterateNodesByType(type) {
  ...
  return nodesByType.getValues(type).map(nodeId => getNode(nodeId)).filter(Boolean);
}
